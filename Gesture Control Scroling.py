...
def detect(hand, landmarks):
    fingers = [i for i in [8, 12, 16, 20] if landmarks.landmark[i].y < landmarks.landmark[i-2].y]
    tip, ip = landmarks.landmark[4], landmarks.landmark[3]
    if hand == "Right" and tip.x > ip.x or hand == "Left" and tip.x < ip.x:
        fingers.append(1)
    return "scroll_up" if len(fingers) == 5 else "scroll_down" if len(fingers) == 0 else "none"

cap = cv2.VideoCapture(0)
cap.set(3, W); cap.set(4, H)
print("Gestures: Scroll Active(Open palm = Scroll Up | Fist = Scroll Down | Press 'q' to quit)")
